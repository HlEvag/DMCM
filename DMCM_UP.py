...
with open(os.path.join('datasets',  'Chikusei_imdb_128_2.pickle'), 'rb') as handle:
...
test_data = '/home/hulei/Datasets/PaviaU.mat'
test_label = '/home/hulei/Datasets/PaviaU_gt.mat'
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    gt1=GroundTruth.reshape(610,340)
...
class HyperCLR(nn.Module):
    ...
    def forward(self, x, domain='source'):
        ...
        # x30 = self.conv3(AP2)
        # # x31 = self.Ar3(x30)
        # x31 = self.ghost_cheaper3d_3(x30)
        # x31 = self.Att3(x31)
        # x32 = self.conv31(x30)
        # x32 = self.bn3(x32)
        # x33 = x32+x31
        ...
        # x = self.conv2(x)
        # x = self.Ar2(x)
        # x = self.ghost_cheaper3d_2(x)
        # x = self.AP2(x)
        
        # # x = self.conv3(x)
        # # x = self.Ar3(x)
        # x = self.ghost_cheaper3d_3(x)
        # AP3 = self.AP3(x33)
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(10000):  # EPISODE = 10000
        ...
        if episode % 2 == 0:
            ...
            loss = a*(f_loss + f_loss2) + b*domain_loss# 0.01
        ...
        # target domain few-shot + domain adaptation
            ...
            domain_loss = Adapt_loss(outputs_sou, outputs_tar)
            # total_loss = fsl_loss + domain_loss
            loss = a*(f_loss + f_loss2) + b*domain_loss# 0.01 0.5=78;0.25=80;0.01=80
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,  "classification_results/UP_new_mmd{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))

file_name = r"classification_results/UP_9x9_MD1_10_self_new_mmmd_report.txt"
